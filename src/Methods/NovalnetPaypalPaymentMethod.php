...
class NovalnetPaypalPaymentMethod extends PaymentMethodService
{
    ...
    public function isActive():bool
    {
       if ($this->configRepository->get('Novalnet.novalnet_paypal_payment_active') == 'true') {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_paypal_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_paypal_maximum_order_amount'));
    ...
    {   
        $name = trim($this->configRepository->get('Novalnet.novalnet_paypal_payment_name'));
    ...
    {
        $description = trim($this->configRepository->get('Novalnet.novalnet_paypal_description'));
        return ($description ? $description : sprintf($this->paymentHelper->getTranslatedText('redirectional_payment_description'),$this->paymentHelper->getTranslatedText('novalnet_paypal')));
    }
